...
class Indicators(object):
    ...
    def get_states(self, quotes):
        self.quotes = quotes
        # print len(quotes)
        self.state = (self.crossover_indicator(self.quotes, 9, 100),
                      self.AO_thresh(self.quotes, 5, 34),
                        self.crossrover(self.quotes, 26),
                        self.RSI(self.quotes, 14, 20))
    ...
    def crossover_indicator(self, q, x, y):
        if self.moving_average(x, q[-x:]) < self.moving_average(y, q[-y:]):
            ...
            print "random! crossrover indic"
            number = random.randint(1, 2)
            if number == 2:
                number = -1
            return number

    def crossrover(self, q, x):
        if q[-1] < self.moving_average(x, q[-x:]):
            return 1 
        else: return -1
    def AO_thresh(self, q, x, y):
        if self.moving_average(x,q[-x:]) == 0 or self.moving_average(y,q[-y:])\
                                                            == 0:
            return 0
        thresh_var=self.moving_average(x,q[-x:])-self.moving_average(y,q[-y:])
        self.logger.debug('AO: {}'.format(thresh_var))
        if thresh_var < 0:
            if thresh_var < -0.00050:
                return -1
        elif thresh_var > 0:
            if thresh_var > 0.00050:
                return 1
